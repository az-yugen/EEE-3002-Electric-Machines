...
class App(ctk.CTk):
    def __init__(self):
        ...
        # POWER DICTIONARY (for Sankey diagram)
        self.power_dict = {
            'sources': ['Input'],
            'targets': ['Output 1', 'Output 2'],
            'values': [100, 50, 50]  # Example values
        }

        # Bind param_dict variables to update_data
        self.param_dict['field_r'].trace_add('write', self.update_data)
        self.param_dict['load_s'].trace_add('write', self.update_data)
