...
def interp():
    ...
    x_new = np.round(np.arange(np.min(x_values), np.max(x_values)+0.001, 0.001), decimals=4)
    # x_new = np.round(np.arange(np.min(x_values), 10, 0.1), decimals=2)
    y_new = np.round(np.interp(x_new, x_values, y_values), decimals=4)

    write_csv('p52_occ_interp.txt', np.column_stack((x_new, y_new)))
